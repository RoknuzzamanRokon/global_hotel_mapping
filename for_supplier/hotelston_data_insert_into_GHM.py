...
def update_global_hotel_mapping(supplier, unica_id):
     ...
     provider_mappings = {
         ...
         "TravelBoutiqueOnline": ["tbohotel", "tbohotel_a", "tbohotel_b", "tbohotel_c", "tbohotel_d", "tbohotel_e"],
     ...
     for provider_code, ids in provider_records.items():
         if provider_code in provider_mappings:
             for column_name, provider_id in zip(provider_mappings[provider_code], ids):
                 # Ensure the column only gets a value if it is empty
                 values_to_update.setdefault(column_name, provider_id)

     try:
         existing_record = session.query(global_hotel_mapping).filter(global_hotel_mapping.c.tbohotel == unica_id).first()
     ...
     if existing_record:
         ...
         final_update_values["mapStatus"] = "D-Done"
     ...
     for column, value in values_to_update.items():
         ...
         if value is not None and not current_val:
             ...
             print(f"Skipping update for {column}, existing value: {current_val}")

     if final_update_values:
         query = update(global_hotel_mapping).where(global_hotel_mapping.c.tbohotel == unica_id).values(**final_update_values)
...
def update_and_save_function(supplier_code, file_path):
    ...
    for hotel_id in hotel_id_list[:]:
        try:
            ...
            file = os.path.join(FILE_PATH, f"error_{supplier_code}_hotel_id_list.txt")
